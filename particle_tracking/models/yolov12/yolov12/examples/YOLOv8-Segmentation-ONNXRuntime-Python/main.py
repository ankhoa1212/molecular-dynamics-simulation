...
class YOLOv8Seg:
    ...
    def __init__(self, onnx_model):
        ...
        self.ndtype = (
            np.half if self.session.get_inputs()[0].type == "tensor(float16)" else np.single
        )
    ...
    def process_mask(self, protos, masks_in, bboxes, im0_shape):
        ...
        masks = (
            np.matmul(masks_in, protos.reshape((c, -1))).reshape((-1, mh, mw)).transpose(1, 2, 0)
        )  # HWN
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source", type=str, default=str(ASSETS / "bus.jpg"), help="Path to input image"
    )
