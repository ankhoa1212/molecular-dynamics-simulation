    ...
    def __init__(
        ...
        if isinstance(
            ...
            def _forward(x):
                ...
                return (
                    self.forward(x)[0] if isinstance(m, (Segment, Pose, OBB)) else self.forward(x)
                )
...
class PoseModel(DetectionModel):
    ...
    def __init__(
        self, cfg="yolov8n-pose.yaml", ch=3, nc=None, data_kpt_shape=(None, None), verbose=True
    ):
...
def temporary_modules(modules=None, attributes=None):
    ...
    try:
        ...
        for old, new in attributes.items():
            ...
            setattr(
                import_module(old_module), old_attr, getattr(import_module(new_module), new_attr)
            )
