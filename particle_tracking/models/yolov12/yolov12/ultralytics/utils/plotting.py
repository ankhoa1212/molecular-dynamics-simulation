...
class Annotator:
    ...
    def circle_label(
        ...
        # Get the text size
        text_size = cv2.getTextSize(str(label), cv2.FONT_HERSHEY_SIMPLEX, self.sf - 0.15, self.tf)[
            0
        ]
    ...
    def box_label(
        self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False
    ):
        """
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                ...
                self.draw.text(
                    (p1[0], p1[1] - h if outside else p1[1]), label, fill=txt_color, font=self.font
                )
        else:  # cv2
            if rotated:
    ...
    def kpts(
        self, kpts, shape=(640, 640), radius=None, kpt_line=True, conf_thres=0.25, kpt_color=None
    ):
        """
            ...
            if x_coord % shape[1] != 0 and y_coord % shape[0] != 0:
                if len(k) == 3:
                    ...
                    if conf < conf_thres:
                        continue
                cv2.circle(
                    self.im, (int(x_coord), int(y_coord)), radius, color_k, -1, lineType=cv2.LINE_AA
                )
    ...
    def text(self, xy, text, txt_color=(255, 255, 255), anchor="top", box_style=False):
        ...
        if self.pil:
            ...
            if box_style:
                ...
                txt_color = (255, 255, 255)
            cv2.putText(
                self.im, text, xy, 0, self.sf, txt_color, thickness=self.tf, lineType=cv2.LINE_AA
            )
    ...
    def display_objects_labels(self, im0, text, txt_color, bg_color, x_center, y_center, margin):
        ...
        cv2.putText(
            im0, text, (text_x, text_y), 0, self.sf, txt_color, self.tf, lineType=cv2.LINE_AA
        )
    ...
    def sweep_annotator(
        self, line_x=0, line_y=0, label=None, color=(221, 0, 186), txt_color=(255, 255, 255)
    ):
        """
    ...
    def plot_distance_and_line(
        ...
        cv2.rectangle(
            self.im, (15, 25), (15 + text_width_m + 20, 25 + text_height_m + 20), line_color, -1
        )
