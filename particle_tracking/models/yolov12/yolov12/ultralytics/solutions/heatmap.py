...
class Heatmap(ObjectCounter):
    ...
    def __init__(self, **kwargs):
        ...
        # store colormap
        self.colormap = (
            cv2.COLORMAP_PARULA if self.CFG["colormap"] is None else self.CFG["colormap"]
        )
        self.heatmap = None
    ...
    def generate_heatmap(self, im0):
        ...
        for box, track_id, cls in zip(self.boxes, self.track_ids, self.clss):
            ...
            if self.region is not None:
                ...
                self.count_objects(
                    current_centroid, track_id, prev_position, cls
        ...
        if self.track_data.id is not None:
            im0 = cv2.addWeighted(
                ...
                0.5,
                cv2.applyColorMap(
                    cv2.normalize(self.heatmap, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8),
                    self.colormap,
                ),
                0.5,
                0,
