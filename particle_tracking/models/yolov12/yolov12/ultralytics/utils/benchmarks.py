...
def benchmark(
    ...
    for i, (name, format, suffix, cpu, gpu, _) in enumerate(zip(*export_formats().values())):
        ...
        try:
            ...
            y.append(
                [name, "✅", round(file_size(filename), 1), round(metric, 4), round(speed, 2), fps]
            )
    ...
    df = pd.DataFrame(
        y, columns=["Format", "Status❔", "Size (MB)", key, "Inference time (ms/im)", "FPS"]
    )
            ...
            for line in file:
                try:
                    ...
                    if not Path(proj_version).exists():
                        self.rf.workspace(workspace).project(project).version(version).download(
                            "yolov8"
                        )
                    else:
...
class ProfileModels:
    ...
    def profile_tensorrt_model(self, engine_file: str, eps: float = 1e-3):
        ...
        num_runs = max(
            round(self.min_time / (elapsed + eps) * self.num_warmup_runs), self.num_timed_runs * 50
        )
    ...
    def profile_onnx_model(self, onnx_file: str, eps: float = 1e-3):
        ...
        # Compute number of runs as higher of min_time or num_timed_runs
        num_runs = max(
            round(self.min_time / (elapsed + eps) * self.num_warmup_runs), self.num_timed_runs
        )
