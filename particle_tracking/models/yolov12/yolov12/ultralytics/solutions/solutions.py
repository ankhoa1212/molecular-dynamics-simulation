...
class BaseSolution:
    ...
    def __init__(self, IS_CLI=False, **kwargs):
        ...
        self.track_add_args = {  # Tracker additional arguments for advance configuration
            k: self.CFG[k]
            for k in ["verbose", "iou", "conf", "device", "max_det", "half", "tracker"]
        }
        ...
        if IS_CLI and self.CFG["source"] is None:
            ...
            LOGGER.warning(
                f"⚠️ WARNING: source not provided. using default source {ASSETS_URL}/{d_s}"
            )
    ...
    def extract_tracks(self, im0):
        ...
        self.tracks = self.model.track(
            source=im0, persist=True, classes=self.CFG["classes"], **self.track_add_args
        )
