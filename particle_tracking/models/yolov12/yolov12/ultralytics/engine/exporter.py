...
def export_formats():
    ...
    x = [
        ...
        [
            ...
            False,
            True,
            ["batch", "dynamic", "half", "int8", "simplify"],
        ],
...
class Exporter:
    ...
    def __call__(self, model=None) -> str:
        ...
        self.pretty_name = Path(self.model.yaml.get("yaml_file", self.file)).stem.replace(
            "yolo", "YOLO"
        )
        ...
        if is_tf_format:  # TensorFlow formats
            ...
            if tflite:
                f[7], _ = self.export_tflite(
                    keras_model=keras_model, nms=False, agnostic_nms=self.args.agnostic_nms
                )
            if edgetpu:
                f[8], _ = self.export_edgetpu(
                    tflite_model=Path(f[5]) / f"{self.file.stem}_full_integer_quant.tflite"
                )
            if tfjs:
    ...
    def export_onnx(self, prefix=colorstr("ONNX:")):
        ...
        output_names = (
            ["output0", "output1"] if isinstance(self.model, SegmentationModel) else ["output0"]
        )
    ...
    def export_openvino(self, prefix=colorstr("OpenVINO:")):
        ...
        def serialize(ov_model, file):
            ...
            ov_model.set_rt_info(
                [v.replace(" ", "_") for v in self.model.names.values()], ["model_info", "labels"]
            )
    ...
    def export_paddle(self, prefix=colorstr("PaddlePaddle:")):
        """YOLO Paddle export."""
        check_requirements(
            ("paddlepaddle-gpu" if torch.cuda.is_available() else "paddlepaddle", "x2paddle")
        )
    ...
    def export_ncnn(self, prefix=colorstr("NCNN:")):
        ...
        if not pnnx.is_file():
            ...
            unzip_dir = safe_download(
                f"https://github.com/pnnx/pnnx/releases/download/{release}/{asset}", delete=True
            )
    ...
    def export_coreml(self, prefix=colorstr("CoreML:")):
        ...
        if self.model.task == "classify":
            classifier_config = (
                ct.ClassifierConfig(list(self.model.names.values())) if self.args.nms else None
            )
            model = self.model
        ...
        bits, mode = (
            (8, "kmeans") if self.args.int8 else (16, "linear") if self.args.half else (32, None)
        )
        if bits < 32:
            if "kmeans" in mode:
                check_requirements(
    ...
    def export_engine(self, dla=None, prefix=colorstr("TensorRT:")):
        ...
        if self.args.dynamic:
            ...
            max_shape = (
                *shape[:2],
                *(int(max(1, workspace) * d) for d in shape[2:]),
            )  # max input shape
            for inp in inputs:
    ...
    def export_saved_model(self, prefix=colorstr("TensorFlow SavedModel:")):
        ...
        if self.args.int8:
            ...
            for file in f.rglob("*_dynamic_range_quant.tflite"):
                file.rename(
                    file.with_name(file.stem.replace("_dynamic_range_quant", "_int8") + file.suffix)
                )
    ...
    def export_edgetpu(self, tflite_model="", prefix=colorstr("Edge TPU:")):
        ...
        if (
            ...
            for c in (
                ...
                subprocess.run(
                    c if is_sudo_available() else c.replace("sudo ", ""), shell=True, check=True
                )
        ver = (
    ...
    def export_tfjs(self, prefix=colorstr("TensorFlow.js:")):
        ...
        quantization = (
            "--quantize_float16" if self.args.half else "--quantize_uint8" if self.args.int8 else ""
        )
    ...
    def export_imx(self, prefix=colorstr("IMX:")):
        ...
        check_requirements(
            ("model-compression-toolkit==2.1.1", "sony-custom-layers==0.2.0", "tensorflow==2.12.0")
        )
        check_requirements(
        ...
        quant_model = (
            mct.gptq.pytorch_gradient_post_training_quantization(  # Perform Gradient-Based Post Training Quantization
                ...
                gptq_config=mct.gptq.get_pytorch_gptq_config(
                    n_epochs=1000, use_hessian_based_weights=False
                ),
                core_config=config,
