...
def get_cfg(
    cfg: Union[str, Path, Dict, SimpleNamespace] = DEFAULT_CFG_DICT, overrides: Dict = None
):
    """
...
def get_save_dir(args, name=None):
    ...
    if getattr(args, "save_dir", None):
        ...
        project = (
            args.project
            or (ROOT.parent / "tests/tmp/runs" if TESTS_RUNNING else RUNS_DIR) / args.task
        )
...
def handle_yolo_solutions(args: List[str]) -> None:
    ...
    if s_n == "inference":
        ...
        solution = getattr(solutions, cls)(
