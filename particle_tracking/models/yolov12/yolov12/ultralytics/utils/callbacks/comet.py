...
def _fetch_trainer_metadata(trainer):
    ...
    return dict(
        curr_epoch=curr_epoch, curr_step=curr_step, save_assets=save_assets, final_epoch=final_epoch
    )
...
def _log_plots(experiment, trainer):
    ...
    plot_filenames = None
    if (
        isinstance(trainer.validator.metrics, SegmentMetrics)
        and trainer.validator.metrics.task == "segment"
    ):
        plot_filenames = [
...
def _log_model(experiment, trainer):
    ...
    experiment.log_model(
        model_name, file_or_folder=str(trainer.best), file_name="best.pt", overwrite=True
    )
...
def on_train_epoch_end(trainer):
    ...
    experiment.log_metrics(
        trainer.label_loss_items(trainer.tloss, prefix="train"), step=curr_step, epoch=curr_epoch
    )
