...
def generate_stability_plot(data_dir, pattern, verbose):
    ...
    scatter = plt.scatter(epsilons, num_molecules, c=avg_psi6, 
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate hexatic stability plot from LAMMPS trajectory files.")
    parser.add_argument("data_dir", nargs="?", default="/home/austin/git/molecular-dynamics-simulation/lammps-scripts/1.0_temp", help="Path to folder containing .lammpstrj files (default: ./data)")
    parser.add_argument("--pattern", default="*.in_*_*.lammpstrj", help="Filename pattern inside the folder (default: %(default)s)")
