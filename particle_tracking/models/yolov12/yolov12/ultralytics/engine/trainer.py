...
class BaseTrainer:
    ...
    def _setup_scheduler(self):
        ...
        if self.args.cos_lr:
            ...
            self.lf = (
                lambda x: max(1 - x / self.epochs, 0) * (1.0 - self.args.lrf) + self.args.lrf
            )  # linear
    ...
    def _do_train(self, world_size=1):
        ...
        nw = (
            max(round(self.args.warmup_epochs * nb), 100) if self.args.warmup_epochs > 0 else -1
        )  # warmup iterations
        last_opt_step = -1
        ...
        while True:
            ...
            self.lr = {
                f"lr/pg{ir}": x["lr"] for ir, x in enumerate(self.optimizer.param_groups)
            }  # for loggers
