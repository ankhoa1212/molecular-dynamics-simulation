    ...
    for line in requires:
        ...
        if line and not line.startswith("#"):
            ...
            if match := re.match(r"([a-zA-Z0-9-_]+)\s*([<>!=~]+.*)?", line):
                requirements.append(
                    SimpleNamespace(name=match[1], specifier=match[2].strip() if match[2] else "")
                )
...
def check_requirements(
    ...
    if isinstance(requirements, Path):  # requirements.txt file
        ...
        requirements = [
            f"{x.name}{x.specifier}" for x in parse_requirements(file) if x.name not in exclude
        ]
...
def check_is_path_safe(basedir, path):
    ...
    return (
        ...
        and path_resolved.parts[: len(base_dir_resolved.parts)] == base_dir_resolved.parts
    )
...
def collect_system_info():
    ...
    for r in parse_requirements(package="ultralytics"):
        try:
            ...
            is_met = (
                "✅ " if check_version(current, str(r.specifier), name=r.name, hard=True) else "❌ "
            )
