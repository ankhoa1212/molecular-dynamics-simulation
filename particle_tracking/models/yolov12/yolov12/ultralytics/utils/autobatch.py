...
def autobatch(model, imgsz=640, fraction=0.60, batch_size=DEFAULT_CFG.batch, max_num_obj=1):
    ...
    try:
        ...
        p = np.polyfit(
            np.log(fit_x), np.log(fit_y), deg=1
        )  # first-degree polynomial fit in log space
        b = int(
            round(np.exp((np.log(f * fraction) - p[1]) / p[0]))
        ...
        if b < 1 or b > 1024:  # b outside of safe range
            LOGGER.info(
                f"{prefix}WARNING ⚠️ batch={b} outside safe range, using default batch-size {batch_size}."
            )
            b = batch_size
