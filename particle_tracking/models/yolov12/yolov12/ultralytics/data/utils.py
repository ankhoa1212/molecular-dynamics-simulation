...
def polygons2masks(imgsz, polygons, color, downsample_ratio=1):
    ...
    """
    return np.array(
        [polygon2mask(imgsz, [x.reshape(-1)], color, downsample_ratio) for x in polygons]
    )
...
def polygons2masks_overlap(imgsz, segments, downsample_ratio=1):
    ...
    for si in range(len(segments)):
        mask = polygon2mask(
            imgsz, [segments[si].reshape(-1)], downsample_ratio=downsample_ratio, color=1
        )
    ...
    def get_json(self, save=False, verbose=False):
        ...
        for split in "train", "val", "test":
            ...
            if path is None:  # no split
                continue
            files = [
                f for f in Path(path).rglob("*.*") if f.suffix[1:].lower() in IMG_FORMATS
