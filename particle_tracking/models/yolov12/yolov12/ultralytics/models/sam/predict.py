...
class Predictor(BasePredictor):
    ...
    def prompt_inference(
        self, im, bboxes=None, points=None, labels=None, masks=None, multimask_output=False
    ):
        """
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        for masks, orig_img, img_path in zip([pred_masks], orig_imgs, self.batch[0]):
            ...
            results.append(
                Results(orig_img, path=img_path, names=names, masks=masks, boxes=pred_bboxes)
            )
...
class SAM2VideoPredictor(SAM2Predictor):
            ...
            if points is not None:
                for i in range(len(points)):
                    self.add_new_prompts(
                        obj_id=i, points=points[[i]], labels=labels[[i]], frame_idx=frame
                    )
    ...
    def _run_memory_encoder(
        ...
        return (
            maskmem_features.to(dtype=torch.float16, device=self.device, non_blocking=True),
            maskmem_pos_enc,
        )
