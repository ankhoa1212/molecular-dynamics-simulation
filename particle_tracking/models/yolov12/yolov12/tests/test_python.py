...
@pytest.mark.skipif(
    is_github_action_running(), reason="No auth https://github.com/JuanBindez/pytubefix/issues/166"
)
def test_youtube():
...
def test_train_scratch():
    ...
    model = YOLO(CFG)
    model.train(
        data="coco8.yaml", epochs=2, imgsz=32, cache="disk", batch=-1, close_mosaic=1, name="model"
    )
    model(SOURCE)
...
def test_train_pretrained():
    ...
    model.train(
        data="coco8-seg.yaml", epochs=1, imgsz=32, cache="ram", copy_paste=0.5, mixup=0.5, name=0
    )
    model(SOURCE)
...
def test_model_tune():
    ...
    YOLO("yolo11n-pose.pt").tune(
        data="coco8-pose.yaml", plots=False, imgsz=32, epochs=1, iterations=2, device="cpu"
    )
